...
func storeHashes(conn net.Conn, hashes map[string][]int, encoder *gob.Encoder) {
...
func updateServer(conn net.Conn, action string, filePath string, client *Client, encoder *gob.Encoder) {
...
func monitorDirectory(conn net.Conn, directory string, server *Client, encoder *gob.Encoder) {
	...
	for {
		...
		case event, ok := <-watcher.Events:
			...
			if event.Op&fsnotify.Create == fsnotify.Create {
				...
				updateServer(conn, "create", event.Name, server, encoder)
			}
			if event.Op&fsnotify.Remove == fsnotify.Remove {
				fmt.Println("File deleted:", event.Name)
				updateServer(conn, "delete", event.Name, server, encoder)
			}
...
func queryHash(conn net.Conn, hash int, encoder *gob.Encoder, decoder *gob.Decoder) ([]string, error) {
...
func main() {
	...
	storeHashes(conn, initialHashes, encoder)

	server := NewClient(filePaths)
	go monitorDirectory(conn, directory, server, encoder)
	...
	for {
		...
		case 1:
			...
			queryHash(conn, hash, encoder, decoder)
		...
		case 2:
			...
			ips, err := queryHash(conn, hash, encoder, decoder)
			if err != nil {
			...
			if err != nil {
				log.Println("Invalid address:", ips[0])
				continue
			}

			donwload(peerPort, hash, host, filePath)
