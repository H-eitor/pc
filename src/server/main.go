...
func (s *Server) handleConnection(conn net.Conn) {
	...
	for {
		...
		switch requestType {
		case "store":
			s.handleStoreRequest(conn, decoder, encoder)
		case "create":
			s.handleCreateRequest(conn, decoder, encoder)
		case "delete":
			s.handleDeleteRequest(conn, decoder, encoder)
		case "query":
			s.handleQueryRequest(conn, decoder, encoder)
		default:
...
func (s *Server) handleStoreRequest(conn net.Conn, decoder *gob.Decoder, encoder *gob.Encoder) {
...
func (s *Server) handleCreateRequest(conn net.Conn, decoder *gob.Decoder, encoder *gob.Encoder) {
	var fileHash int
...
func (s *Server) handleDeleteRequest(conn net.Conn, decoder *gob.Decoder, encoder *gob.Encoder) {
	var fileHash int
	if err := decoder.Decode(&fileHash); err != nil {
		log.Println("Error decoding file hash:", err)
		return
...
func (s *Server) handleQueryRequest(conn net.Conn, decoder *gob.Decoder, encoder *gob.Encoder) {
	var hash int
	...
	if err := encoder.Encode(ips); err != nil {
